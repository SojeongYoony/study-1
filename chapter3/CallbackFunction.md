...
// reflection 어떤 메소드를 호출할 수 있는 기능

// 자바 callback의 예시로
// spring controller에서 service를 쓸 때를 알려주심
// field에 service 객체를 만드는데 이것을 어떻게 하느냐.
// spring에서 내부적으로 static으로 객체를 생성함
// private이면 다른 클래스에 접근하지 못 하는데, 어떻게 하느냐 ==> 내부적으로 의미없음 접근가능하게끔 구현되어 있음.
...
* 첫 번째 요소는 마지막 요소가 되며 마지막 요소는 첫 번째 요소가 된다.
  ...
  자신의 생각으로 구현해보기  
  현재 순환 인덱스를 반환하기  ===> forEach가 돌다가 끝난 지점의 index 반환하는거
